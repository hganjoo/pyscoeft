    ...
    for i in range(-1, ncells_1d - 1):
        for j in range(-1, ncells_1d - 1):
            for k in range(-1, ncells_1d - 1):
                ...
                if abs(dLdpi) > 1e-6:
                    pi[i, j, k] = pi_old - w * L / dLdpi
                    if pi[i,j,k]>1e6:print('overshot: ',b[i,j,k],i,j,k,pi[i,j,k],dLdpi,L,av,bv,cv)
                else:
                    pi[i, j, k] = -0.5*bv/av
    ...
    for i in range(-1, ncells_1d - 1):
        for j in range(-1, ncells_1d - 1):
            for k in range(-1, ncells_1d - 1):
                ...
                # Vertex seed: best starting point regardless of disc sign
                pi_cur = -bv / (two * av)
                scale = abs(bv / av) + np.float32(1e-6)

                for _ in range(10):
                    L = av * pi_cur**2 + bv * pi_cur + cv
                    if abs(L) < np.float32(1e-7):
                        break
                    dLdpi = two * av * pi_cur + bv
                    if abs(dLdpi) < np.float32(1e-10):
                        break
                    step = L / dLdpi
                    if abs(step) > np.float32(0.5) * scale:
                        step = np.sign(step) * np.float32(0.5) * scale
                    pi_cur -= step
    ...
    for i in range(-1, ncells_1d - 1):
        for j in range(-1, ncells_1d - 1):
            for k in range(-1, ncells_1d - 1):
                ...
                cv = lin - onebyfour*C4*q2offd/(aH2) - rhs[i,j,k]
